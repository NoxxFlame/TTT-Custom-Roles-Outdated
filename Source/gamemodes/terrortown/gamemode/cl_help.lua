...
CreateClientConVar("ttt_role_symbols", "0", true, true)
...
function HELPSCRN:Show()
    ...
    cb = dgui:CheckBox(GetTranslation("set_symbols"), "ttt_role_symbols")
    cb:SetTooltip(GetTranslation("set_symbols_tip"))
...
function HELPSCRN:CreateTutorial(parent)
    ...
    bar.UpdateText = function(s)
        s.Label:SetText(Format("%02d / %02d", s.m_iValue, s.m_iMax))
    end
    ...
    bnext.DoClick = function()
        if tut.current < tutorial_pages then
            ...
            tut:SetImage(GetImagePath(tut.current))
