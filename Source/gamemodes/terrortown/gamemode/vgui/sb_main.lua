    ...
    if DetectiveMode() then
        ...
        if p:IsSpec() and (not p:Alive()) then
            ...
            elseif p:GetNWBool("body_found", false) then
                ...
                -- To terrorists and monsters, missing players show as alive
                if client:IsSpec() or client:IsActiveTraitorTeam() or client:IsActiveMonsterTeam() or ((GAMEMODE.round_state ~= ROUND_ACTIVE) and client:IsTerror()) then
