...
local smg_weapon_replace = {
	-- MAC 10 (Weight: 3)
	"weapon_zm_mac10",
	"weapon_zm_mac10",
	"weapon_zm_mac10",
	-- H.U.G.E-249 (Weight: 4)
	"weapon_zm_sledge",
	"weapon_zm_sledge",
	"weapon_zm_sledge",
	"weapon_zm_sledge",
...
function ents.TTT.ReplaceWeaponsFromPools()
	local counter = 0
	for _, ent in pairs(ents.FindByClass("weapon_*")) do
		if ent.AllowDelete == false then
		else
			...
			local found = false
			if not found then
				for k, v in pairs(assault_weapon_replace) do
					if cls == v then
						found = true
						rpl = assault_weapon_replace[math.random(1, #assault_weapon_replace)]
						break
					end
				end
			end
			if not found then
				for k, v in pairs(sniper_weapon_replace) do
					if cls == v then
						found = true
						rpl = sniper_weapon_replace[math.random(1, #sniper_weapon_replace)]
						break
					end
				end
			end
			if not found then
				for k, v in pairs(smg_weapon_replace) do
					if cls == v then
						found = true
						rpl = smg_weapon_replace[math.random(1, #smg_weapon_replace)]
						break
					end
				end
			end
			if not found then
				for k, v in pairs(shotgun_weapon_replace) do
					if cls == v then
						found = true
						rpl = shotgun_weapon_replace[math.random(1, #shotgun_weapon_replace)]
						break
					end
				end
			end
			if not found then
				for k, v in pairs(pistol_weapon_replace) do
					if cls == v then
						found = true
						rpl = pistol_weapon_replace[math.random(1, #pistol_weapon_replace)]
						break
					end
				end
			end
	...
	print("Replaced " .. counter .. " weapons.")
end
