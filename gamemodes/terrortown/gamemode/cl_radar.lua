...
function RADAR:Draw(client)
	...
	-- C4 warnings
	if self.bombs_count ~= 0 and (client:IsActiveTraitor() or client:IsActiveHypnotist() or client:IsActiveAssassin()) then
	...
	for k, tgt in pairs(RADAR.targets) do
		...
		if scrpos.visible then
			...
			elseif client:GetZombie() or client:GetVampire() then
				if role == ROLE_ZOMBIE or role == ROLE_VAMPIRE or role == ROLE_GLITCH then
