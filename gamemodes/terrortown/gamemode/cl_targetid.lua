...
function GM:PostDrawTranslucentRenderables()
	...
	for i = 1, #plys do
		ply = plys[i]
		if ply:IsActive() and ply ~= client then
			pos = ply:GetPos()
			pos.z = pos.z + 74
...
function GM:HUDDrawTargetID()
	...
	if target_innocent or target_detective or target_glitch or target_mercenary or target_phantom or target_traitor or target_assassin or target_hypnotist or target_vampire or target_zombie or target_jester or target_swapper or target_fellow_traitor or target_fellow_zombie then
	...
	elseif target_fellow_zombie or (target_glitch and client:GetRole() == ROLE_ZOMBIE) then
		...
		clr = Color(69, 97, 0, 200)
	elseif target_corpse and (client:IsActiveDetective() or client:IsActiveTraitor() or client:IsActiveMercenary() or client:IsActiveZombie() or client:IsActiveVampire() or client:IsActiveHypnotist() or client:IsActiveAssassin()) and CORPSE.GetCredits(ent, 0) > 0 then
